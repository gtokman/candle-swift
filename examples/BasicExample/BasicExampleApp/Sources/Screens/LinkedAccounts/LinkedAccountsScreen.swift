...
struct LinkedAccountsScreen: View {
    ...
    @State var accountToUnlink: Models.LinkedAccount? = nil

    var gradientBackground: Color {
        colorScheme == .dark ? .black : .white
    }

    func makeTitle(for linkedAccount: Models.LinkedAccount?) -> String {
        if let linkedAccount {
            "Unlink \(linkedAccount.service.name)"
        } else {
            "N/A"
        }
    }
    ...
    var body: some View {
        List {
            ForEach(linkedAccounts) { linkedAccount in
                LinkedAccountRow(linkedAccount: linkedAccount)
                    .swipeActions {
                        Button("Unlink") {
        ...
        .confirmationDialog(
            ...
            Button(
                makeTitle(for: accountToUnlink),
                role: .destructive,
                action: {
                    if let accountToUnlink {
                        unlinkAction(accountToUnlink)
                    } else {
    ...
    private func unlinkAction(_ linkedAccount: Models.LinkedAccount) {
        Task {
            defer { isLoading = false }
            do {
                isLoading = true
                linkedAccounts = []
                try await client.unlinkAccount(linkedAccountID: linkedAccount.linkedAccountID)
